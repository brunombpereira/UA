...
Graph* GraphCopy(const Graph* g) {
    ...
    // Mapeia os IDs originais para os IDs copiados
    unsigned int* idMapping = (unsigned int*)malloc(g->numVertices * sizeof(unsigned int));
    ...
    for (; i < g->numVertices; ListMoveToNext(vertices), ++i) {
        ...
        unsigned int originalID = originalVertex->id;

        // Adiciona a cópia do vértice
        struct _Vertex* copiedVertex = (struct _Vertex*)malloc(sizeof(struct _Vertex));
        copiedVertex->id = i;
        copiedVertex->inDegree = originalVertex->inDegree;
        copiedVertex->outDegree = originalVertex->outDegree;
        copiedVertex->edgesList = ListCreate(graphEdgesComparator);

        ListInsert(copy->verticesList, copiedVertex);

        // Atualiza o mapeamento de IDs
        idMapping[originalID] = i;

        List* edges = originalVertex->edgesList;
        ListMoveToHead(edges);

        // Adiciona as arestas do vértice original ao vértice copiado
        unsigned int j = 0;
        for (; j < ListGetSize(edges); ListMoveToNext(edges), ++j) {
            struct _Edge* originalEdge = ListGetCurrentItem(edges);
            struct _Edge* copiedEdge = (struct _Edge*)malloc(sizeof(struct _Edge));
            copiedEdge->adjVertex = idMapping[originalEdge->adjVertex];
            copiedEdge->weight = originalEdge->weight;
            ListInsert(copiedVertex->edgesList, copiedEdge);
        }
    }
...
Graph* GraphFromFile(FILE* f) { 
  assert(f != NULL);
    ...
    fscanf(f, "%u %d %d", &isDigraph, &isWeighted ,&numVertices);
    printf ("isWeighted = %d\n", isWeighted);
...
int GraphRemoveEdge(Graph* g, unsigned int v, unsigned int w) {
    ...
    if (g->isDigraph == 0) {
        ...
        struct _Edge* destEdge = ListGetCurrentItem(destVertex->edgesList);
...
int GraphCheckInvariants(const Graph* g) {
  assert(g != NULL);
    ...
    for (; i < g->numVertices; ListMoveToNext(vertices), i++) {
        struct _Vertex* v = ListGetCurrentItem(vertices);
        assert(v->id == i);
        assert(v->inDegree + v->outDegree <= g->numVertices - 1);
        assert(ListGetSize(v->edgesList) == v->outDegree);
        List* edges = v->edgesList;
        ListMoveToHead(edges);
        unsigned int j = 0;
        for (; j < ListGetSize(edges); ListMoveToNext(edges), j++) {
            struct _Edge* e = ListGetCurrentItem(edges);
            assert(e->adjVertex < g->numVertices);
        }
    }
