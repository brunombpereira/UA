...
int main(void)
{
	...
	while(1)
	{
		...
		delay(10);
		if(i % 20 == 0)
			counter++; //Para poder aumentar a freq de refrescamento dos displays
					   //sem alterar a frequência de atualização do contador (5Hz)
					   //tive de dividir os 200ms(5Hz) desejados pela frequência de
					   //atualização dos displays(20ms = 50Hz)
					   //200=x*20 <=> x = 200/20 = 10
	}
