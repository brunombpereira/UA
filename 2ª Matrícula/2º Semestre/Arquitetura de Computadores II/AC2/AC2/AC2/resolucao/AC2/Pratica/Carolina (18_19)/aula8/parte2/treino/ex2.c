...
int main(void){
	...
	//interuupçoes
	U1STAbits.URXISEL = 0;		//permite interrupçoes para receção
	U1STAbits.UTXSEL = 1;
...
void _int_(24) isr_uart1(void){
	if(U1RXREG == 'L'){
			putc('M');
			putc( (bintoBCD(max)>>4) +'0');
			putc( '.');
			putc( ((bintoBCD(max)) & 0x0F) + '0'); 
			putc('\n');

			putc('m');
			putc( (bintoBCD(min)>>4) +'0');
			putc( '.');
			putc( ((bintoBCD(min)) & 0x0F) + '0'); 
			putc('\n');
...
void _int_(4) isr_T1(void){
	...
	{
		count=0;
		putc( (bintoBCD(voltage)>>4) +'0');
		putc( '.');
		putc( ((bintoBCD(voltage)) & 0x0F) + '0'); 
		putc('\n');
	}
